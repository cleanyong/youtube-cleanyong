...
| 类别 | 项目 | 功能 | 常见开源命令行替代 |
| --- | --- | --- | --- |
| 线 | EdDSA-keygen | 生成/测试 Ed25519 密钥对并导出二进制+Base64 | `ssh-keygen -t ed25519`，`openssl genpkey -algorithm ED25519` |
| 线 | adhoc-chat | 临时 2 人聊天室（WebSocket 服务）-- 中文名：尬聊 | ... |
| 线 | alice-send | 生成 Alice 的 DH 私钥/公钥，可用 Bob 公钥算共享密钥 | `openssl genpkey -genparam -algorithm DH` + `openssl pkeyutl -derive` |
| 线 | alice-sign | 用本地 Ed25519 私钥签 JSON，输出公钥 | `openssl pkeyutl -sign -inkey key.pem -rawin`（Ed25519） |
| 线 | bob-send | 读取 Alice 公钥，生成 Bob DH 密钥并算共享密钥 | 同上 `openssl genpkey`/`pkeyutl -derive` |
| 线 | bob-verify | 用保存的 Alice 公钥验证消息签名 | `openssl pkeyutl -verify`，`openssl dgst -verify -signature sig.bin` |
| 线 | chat-encryption-helper | 基于 DH 握手的聊天 AES-256 加解密助手, 可以使用任何信道来传输，比如email，聊天室，论坛，短信etc | `signal-cli`（端到端加密聊天） |
| 点 | cmd-call-kdbx | KeePass KDBX 搜索/创建/生成 keyfile 的 CLI | `keepassxc-cli show/find` |
| 点 | create-DNA-for-file | 文件/STDIN 做 SHA-256，输出十进制/十六进制大整数 | `sha256sum file`，`openssl dgst -sha256 file` |
| 线 | create-private-key | 生成 DH 私钥/参数（MODP14 等） | `openssl genpkey -algorithm DH -pkeyopt group:modp_2048` |
| 点 or 线 | decrypt-file | AES-256-CBC 解密（密码取 SHA-256），输出 `.clear` | `openssl enc -aes-256-cbc -d -in file.enc -out file.clear -pass pass:...` |
| 线 | dilithium-keygen | 生成/测试 ML-DSA-65/Dilithium3 密钥对 | `openssl genpkey -provider oqsprovider -algorithm dilithium3` |
| 点 | dmg-util | 包装 `hdiutil` 创建加密 APFS DMG | 直接用 `hdiutil create -encryption AES-256 ...` |
| 点 or 线 | encrypt-file | AES-256-CBC 加密（密码取 SHA-256），输出 `.enc` | `openssl enc -aes-256-cbc -salt -in file -out file.enc -pass pass:...` |
| 线 | find-big-prime | 生成大素数/安全素数（Miller–Rabin） | `openssl prime -generate -bits 2048`（加 `-safe` 生成安全素数） |
| 线 | find-big-safe-prime | 并发生成安全素数 p=2q+1 | `openssl prime -generate -bits 2048 -safe` |
| 点 | find-finger-print-for-file | 计算文件 SHA-256 指纹 | `sha256sum file` |
